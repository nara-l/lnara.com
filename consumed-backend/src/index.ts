...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    // Run every 6 hours to prevent Last.fm data loss (200 track limit)
    // Runs at 3 AM, 9 AM, 3 PM, 9 PM UTC daily
    const ny = nowInNY();
    const currentHour = new Date().getUTCHours();
    const runId = `${ny.toLocaleDateString("en-CA", { timeZone: "America/New_York" })}-${currentHour}`;

    console.log(`Scheduled ingest starting at ${new Date().toISOString()}, run ID: ${runId}`);

    // Prevent duplicate runs in the same time slot
    const key = `last-run-${runId}`;
    const already = await env.RUN_STATUS.get(key);
    if (already) {
      console.log(`Skipping - already processed run ${runId}`);
      return;
    }
    ...
    console.log("Running Last.fm and YouTube ingest...");
    const [lfm, yt] = await Promise.allSettled([ingestLastFm(env), ingestYouTubeLikes(env)]);

    const rows: any[] = [];
    let lfmCount = 0, ytCount = 0;

    if (lfm.status === "fulfilled") {
      lfmCount = lfm.value.length;
      rows.push(...lfm.value);
    } else {
      console.error("Last.fm ingest failed:", lfm.reason);
    }

    if (yt.status === "fulfilled") {
      ytCount = yt.value.length;
      rows.push(...yt.value);
    } else {
      console.error("YouTube ingest failed:", yt.reason);
    }

    if (rows.length) {
      await sheets.appendRows(rows as any);
      console.log(`Scheduled ingest completed: ${lfmCount} Last.fm tracks, ${ytCount} YouTube videos, ${rows.length} total new entries`);
    } else {
      console.log("Scheduled ingest completed: No new entries found");
    }

    // Mark this run as completed (expires in 25 hours)
    await env.RUN_STATUS.put(key, new Date().toISOString(), { expirationTtl: 60 * 60 * 25 });
  },
};
